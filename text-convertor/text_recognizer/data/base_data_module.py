...
def load_print_info(data_module_class) -> None :
    """ 
    Load EMNIST data print info
    """
...
def _download_raw_dataset(metadata:Dict , dl_dirname :Path) -> Path :
    dl_dirname.mkdir(parents=True,exsits_ok=True)
    ...
    if sha256!= metadata["sha256"]:
        raise ValueError("Downloaded data file SHA-256 does not match that listed in metadata documents")
...
class BaseDataModule(pl.LightningDataModule):
    ...
    def __init__(self,args:argparse.Namespace=None) -> None :
        ...
        #Making sure to set the variables below in subclass
        self.dims: Tuple[int,...]
